...
export default function BadgeGrid({ userBadges }) {
    ...
    return (
        <Card className="col-span-1 md:col-span-2 lg:col-span-3 border-slate-200 shadow-sm">
            <CardHeader>
                <CardTitle className="text-lg font-semibold text-slate-900">Achievements</CardTitle>
            </CardHeader>
            <CardContent>
                <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-4">
                    {ALL_BADGES.map((badge) => {
                        ...
                        return (
                            <div key={badge.name} className={`flex flex-col items-center p-4 rounded-lg border ${isUnlocked ? 'bg-slate-50 border-slate-200' : 'bg-slate-50/50 border-slate-100 opacity-50'}`}>
