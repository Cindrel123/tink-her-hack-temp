            ...
            <Card key={challenge.id} className={`transition-all duration-300 ${challenge.completed ? 'border-malachite-200 bg-malachite-50/30' : 'hover:shadow-md'}`}>
                <CardHeader className="pb-3">
                    <div className="flex justify-between items-start">
                        ...
                        <Badge variant="secondary" className="bg-malachite-100 text-malachite-700 hover:bg-malachite-200 border-malachite-200">
                ...
                <CardContent>
                    <div className="space-y-3">
                        <div className="flex justify-between text-sm text-malachite-600">
                            <span>Progress</span>
                            <span className="font-medium">{challenge.progress} / {challenge.requirement_value}</span>
                        </div>
                        <Progress value={progressPercent} className="h-2" indicatorColor={challenge.completed ? "bg-malachite-600" : "bg-malachite-950"} />
        ...
        <MainLayout>
            <div className="space-y-8 animate-in fade-in duration-500">
                ...
                <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
                    <div>
                        <h1 className="text-3xl font-bold tracking-tight text-malachite-950">Challenges</h1>
                    ...
                    {/* Streak Card */}
                    <Card className="bg-gradient-to-br from-malachite-50 to-white border-malachite-200 shadow-sm min-w-[200px]">
                        <CardContent className="p-4 flex items-center gap-4">
                            <div className="p-3 bg-malachite-100 rounded-full">
                                <Flame className={`h-6 w-6 ${streak > 0 ? 'text-malachite-500 fill-orange-500 animate-pulse' : 'text-neutral-400'}`} />
                            </div>
                            <div>
                                <div className="text-2xl font-bold text-malachite-950">{streak} Day{streak !== 1 && 's'}</div>
                                <div className="text-xs text-malachite-600 font-medium uppercase tracking-wider">Daily Streak</div>
                            </div>
                        </CardContent>
                    </Card>
                </div>
                ...
                {/* Tabs */}
                <div className="flex gap-2 p-1 bg-malachite-100 rounded-lg w-fit">
                    <button
                        onClick={() => setActiveTab('daily')}
                        className={`px-4 py-2 rounded-md text-sm font-medium transition-all ${activeTab === 'daily'
                            ? 'bg-white text-malachite-950 shadow-sm'
                            : 'text-malachite-500 hover:text-malachite-950'
                            }`}
                    >
                        Daily Challenges
                    </button>
                ...
                {/* Challenges Grid */}
                {loading ? (
                    <div className="text-center py-12 text-malachite-500">Loading challenges...</div>
                ) : (
                    <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
                        {challenges[activeTab].map(renderChallengeCard)}

                        {challenges[activeTab].length === 0 && (
                            <div className="col-span-full text-center py-12 border-2 border-dashed border-malachite-200 rounded-xl">
                                <Trophy className="h-12 w-12 text-malachite-300 mx-auto mb-3" />
                                <h3 className="text-lg font-medium text-malachite-950">All caught up!</h3>
                                <p className="text-malachite-500">Check back later for more challenges.</p>
                            </div>
                        )}
                    </div>
                )}
